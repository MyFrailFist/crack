...
var ApproveAnnctComponent = (function () {
    function ApproveAnnctComponent(listAnnctSvc, postAnnctSvc, editAnnctSvc, userSvc, firebaseAuth, router, route) {
        ...
        this.selectedAnnct = this.dummyAnnct;
    }
    ApproveAnnctComponent.prototype.ngOnInit = function () {
        ...
        this.listAnnctSvc.fetchAnnct(this.user, "unapproved", function (fetchedAnnct) { _this.listOfAnnct = fetchedAnnct.reverse(); });
    ...
    ApproveAnnctComponent.prototype.approveAnnct = function (annct) {
...
var BlockAnnctComponent = (function () {
    ...
    BlockAnnctComponent.prototype.ngOnInit = function () {
        ...
        var userId = this.firebaseAuth.authState.uid;
        this.userSvc.getUser(userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        this.listAnnctSvc.fetchAnnct(this.user, "block", function (fetchedAnnct) { _this.listOfAnnct = fetchedAnnct.reverse(); });
        //Once this.listOfAnnct is updated, ngFor should list out all the announcements through observable.
    };
...
var CreateAnnctComponent = (function () {
    function CreateAnnctComponent(postAnnctSvc, userSvc, firebaseAuth, router, route) {
        this.postAnnctSvc = postAnnctSvc;
        this.userSvc = userSvc;
        this.firebaseAuth = firebaseAuth;
        this.router = router;
    ...
    CreateAnnctComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.userSvc.getUser(this.userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        setTimeout(function () {
...
var MyAnnctComponent = (function () {
    ...
    MyAnnctComponent.prototype.ngOnInit = function () {
        ...
        var userId = this.firebaseAuth.authState.uid;
        this.userSvc.getUser(userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        this.listAnnctSvc.fetchAnnct(this.user, "my", function (fetchedAnnct) { _this.listOfAnnct = fetchedAnnct.reverse(); });
        //Once this.listOfAnnct is updated, ngFor should list out all the announcements through observable.
    };
...
var PersonalAnnctComponent = (function () {
    ...
    PersonalAnnctComponent.prototype.ngOnInit = function () {
        ...
        var userId = this.firebaseAuth.authState.uid;
        this.userSvc.getUser(userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        this.listAnnctSvc.fetchAnnct(this.user, "personal", function (fetchedAnnct) { _this.listOfAnnct = fetchedAnnct.reverse(); });
        //Once this.listOfAnnct is updated, ngFor should list out all the announcements through observable.
    };
...
var PropertyAnnctComponent = (function () {
    ...
    PropertyAnnctComponent.prototype.ngOnInit = function () {
        ...
        var userId = this.firebaseAuth.authState.uid;
        this.userSvc.getUser(userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        this.listAnnctSvc.fetchAnnct(this.user, "property", function (fetchedAnnct) { _this.listOfAnnct = fetchedAnnct.reverse(); });
        //Once this.listOfAnnct is updated, ngFor should list out all the announcements through observable.
    };
...
var UnitAnnctComponent = (function () {
    ...
    UnitAnnctComponent.prototype.ngOnInit = function () {
        ...
        var userId = this.firebaseAuth.authState.uid;
        this.userSvc.getUser(userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        this.listAnnctSvc.fetchAnnct(this.user, "unit", function (fetchedAnnct) { _this.listOfAnnct = fetchedAnnct.reverse(); });
        //Once this.listOfAnnct is updated, ngFor should list out all the announcements through observable.
    };
...
var Announcement = (function () {
    function Announcement(title, html, userId, userName, address, scope) {
        this.title = title;
        this.content = html;
        this.userId = userId;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__post_annct_service__ = __webpack_require__("../../../../../src/app/components/basic/community/announcement/shared/post-annct.service.ts");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__services_user_service__ = __webpack_require__("../../../../../src/app/services/user.service.ts");
...
var EditAnnctService = (function () {
    function EditAnnctService(db, postAnnctSvc, userSvc) {
        this.db = db;
        this.postAnnctSvc = postAnnctSvc;
        this.userSvc = userSvc;
    }
    EditAnnctService.prototype.deleteFromDb = function (annct) {
        if (annct.scope === "property") {
            var propAnnctToDelete = this.db.object("announcements/" + annct.address.estate + "/broadcast/" + annct.refToOtherAnnct);
            propAnnctToDelete.remove();
        }
        else if (annct.scope === "block") {
            var blockAnnctToDelete = this.db.object("announcements/" + annct.address.estate + "/" + annct.address.block + "/broadcast/" + annct.refToOtherAnnct);
            blockAnnctToDelete.remove();
        }
        var deleteFromUserAnnct = this.db.object("announcements/" + annct.address.estate + "/userAnnct/" + annct.refToUserAnnct);
        deleteFromUserAnnct.remove();
    };
    EditAnnctService.prototype.deleteAnnct = function (user, annct, reasonForm) {
        var _this = this;
        if (user.$key === annct.userId) {
            ...
            if (reasonForm == undefined) {
                ...
                this.deleteFromDb(annct);
                var reason_1 = reasonForm.value.reason;
                var reasonTitle_1 = "Your announcement \"" + annct.title + "\" has been deleted/disapproved.";
                //search for the user who posted the annct. And notfy him/her
                this.userSvc.getUser(annct.userId).subscribe(function (userInfo) {
                    return _this.postAnnctSvc.postAnnounce(userInfo, reasonTitle_1, reason_1, "personal", user);
                });
                console.log("gfdgfd");
                return true;
            }
    ...
    EditAnnctService.prototype.editAnnct = function (annct, html, title) {
        ...
        annct.date = __WEBPACK_IMPORTED_MODULE_4_firebase__["database"].ServerValue.TIMESTAMP;
        if (annct.scope === "property") {
            var oldPropAnnct = this.db.object("announcements/" + annct.address.estate + "/broadcast/" + annct.refToOtherAnnct);
            oldPropAnnct.update(annct);
        }
        else if (annct.scope === "block") {
            var oldBlockAnnct = this.db.object("announcements/" + annct.address.estate + "/" + annct.address.block + "/broadcast/" + annct.refToOtherAnnct);
            oldBlockAnnct.update(annct);
        }
        else if (annct.scope === "unit") {
            var oldUnitAnnct = this.db.object("announcements/" + annct.address.estate + "/" + annct.address.block + "/" + annct.address.unit + "/broadcast/");
            oldUnitAnnct.update(annct);
        }
        else if (annct.scope === "personal") {
            var oldPersonAnnct = this.db.object("announcements/" + annct.address.estate + "/" + annct.address.block + "/" + annct.address.unit + "/personal/" + annct.userId);
            oldPersonAnnct.update(annct);
        }
        var oldUserAnnct = this.db.list("announcements/" + annct.address.estate + "/userAnnct/");
        oldUserAnnct.update(annct.refToUserAnnct, annct);
    };
    EditAnnctService.prototype.approveAnnct = function (annct) {
        if (annct.scope === "property") {
            var annctToApprove = this.db.object("announcements/" + annct.address.estate + "/broadcast/" + annct.refToOtherAnnct);
            annctToApprove.update({ approved: true });
        }
        else if (annct.scope === "block") {
            var annctToApprove = this.db.object("announcements/" + annct.address.estate + "/" + annct.address.block + "/broadcast/" + annct.refToOtherAnnct + ")");
            annctToApprove.update({ approved: true });
        }
        var userAnnctToApprove = this.db.object("announcements/" + annct.address.estate + "/broadcast/" + annct.refToUserAnnct);
...
EditAnnctService = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_a = typeof __WEBPACK_IMPORTED_MODULE_1_angularfire2_database__["a" /* AngularFireDatabase */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_1_angularfire2_database__["a" /* AngularFireDatabase */]) === "function" && _a || Object, typeof (_b = typeof __WEBPACK_IMPORTED_MODULE_2__post_annct_service__["a" /* PostAnnctService */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_2__post_annct_service__["a" /* PostAnnctService */]) === "function" && _b || Object, typeof (_c = typeof __WEBPACK_IMPORTED_MODULE_3__services_user_service__["a" /* UserService */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_3__services_user_service__["a" /* UserService */]) === "function" && _c || Object])
], EditAnnctService);
...
var ListAnnctService = (function () {
    ...
    function ListAnnctService(db) {
        this.db = db;
    }
    //Subscribe to data at announcement component.
    ListAnnctService.prototype.fetchAnnct = function (user, scope, exportData) {
        if (scope === "property") {
            this.listOfAnnctDb = this.db.list("announcements/" + user.address.estate + "/broadcast/");
            //this.listOfAnnctDb = this.db.list(`${userId}/`)
            this.listOfAnnctDb.subscribe(function (annctDbList) {
                annctDbList.forEach(function (annct) {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__angular_core__ = __webpack_require__("../../../core/@angular/core.es5.js");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_angularfire2_database__ = __webpack_require__("../../../../angularfire2/database/index.js");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__announcement__ = __webpack_require__("../../../../../src/app/components/basic/community/announcement/shared/announcement.ts");
...
var PostAnnctService = (function () {
    function PostAnnctService(db) {
        this.db = db;
    }
    PostAnnctService.prototype.postAnnounce = function (user, title, html, scope, adminUser) {
        if (adminUser != undefined) {
            this.announceToPost = new __WEBPACK_IMPORTED_MODULE_2__announcement__["a" /* Announcement */](title, html, adminUser.$key, adminUser.name, adminUser.address, scope);
        }
        else if (adminUser == undefined) {
            this.announceToPost = new __WEBPACK_IMPORTED_MODULE_2__announcement__["a" /* Announcement */](title, html, user.$key, user.name, user.address, scope);
        }
        //set date of announcement 
        this.announceToPost.date = __WEBPACK_IMPORTED_MODULE_3_firebase__["database"].ServerValue.TIMESTAMP;
        ...
        if (scope === "property") {
            var dbRef = this.db.list("announcements/" + user.address.estate + "/broadcast/");
            this.bindingAnnct(user, dbRef);
        }
        else if (scope === "block") {
            var dbRef = this.db.list("announcements/" + user.address.estate + "/" + user.address.block + "/broadcast/");
            this.bindingAnnct(user, dbRef);
        }
        else if (scope === "unit") {
            var dbRef = this.db.list("announcements/" + user.address.estate + "/" + user.address.block + "/" + user.address.unit + "/broadcast/");
            this.bindingAnnct(user, dbRef);
        }
        else if (scope === "personal") {
            var dbRef = this.db.list("announcements/" + user.address.estate + "/" + user.address.block + "/" + user.address.unit + "/personal/" + user.$key + "/");
            this.bindingAnnct(user, dbRef);
        }
    };
    //Creating a link between the 2 twin announcements and post them.
    PostAnnctService.prototype.bindingAnnct = function (user, dbRef) {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__shared_convert_event_service__ = __webpack_require__("../../../../../src/app/components/basic/event-calendar/shared/convert-event.service.ts");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__shared_event__ = __webpack_require__("../../../../../src/app/components/basic/event-calendar/shared/event.ts");
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var EventCalendarComponent = (function () {
    function EventCalendarComponent(modal, listEventSvc, addEventSvc, userSvc, authSvc, convertEventSvc) {
        var _this = this;
        this.modal = modal;
        ...
        this.convertEventSvc = convertEventSvc;
        this.view = 'month';
    ...
    EventCalendarComponent.prototype.addEvent = function (eventForm) {
        this.newEvent = new __WEBPACK_IMPORTED_MODULE_9__shared_event__["a" /* userEvent */](eventForm.value.title, eventForm.value.description);
    ...
    EventCalendarComponent.prototype.editEvent = function (index) {
        var changedEvent = this.eventList[index];
    ...
    EventCalendarComponent.prototype.ngOnInit = function () {
        ...
        this.userSvc.getUser(userId).subscribe(function (userInfo) { return _this.user = userInfo; });
        this.listEventSvc.fetchEvent(this.user, function (fetchEvent, calendarEvents) {
            _this.events = calendarEvents;
            _this.eventList = fetchEvent;
            _this.eventsCopy = _this.events;
        });
    };
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__shared_convert_event_service__ = __webpack_require__("../../../../../src/app/components/basic/event-calendar/shared/convert-event.service.ts");
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
EventCalendarModule = __decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["NgModule"])({
        ...
        providers: [
            ...
            __WEBPACK_IMPORTED_MODULE_9__shared_convert_event_service__["a" /* ConvertEventService */]
        ],
...
var AddEventService = (function () {
    function AddEventService(db) {
        this.db = db;
    }
    AddEventService.prototype.addEvent = function (event, user) {
        event.date.dateCreated = __WEBPACK_IMPORTED_MODULE_2_firebase__["database"].ServerValue.TIMESTAMP;
        var eventDbRef = this.db.list("eventCalendar/" + user.address.estate + "/" + user.$key + "/");
        var eventKey = eventDbRef.push(event).key;
        eventDbRef.update(eventKey, { id: eventKey });
    };
    AddEventService.prototype.deleteEvent = function (event, user) {
        var eventDbRef = this.db.object("eventCalendar/" + user.address.estate + "/" + user.$key + "/" + event.id);
        eventDbRef.remove();
    };
    AddEventService.prototype.editEvent = function (event, user) {
        var eventDbRef = this.db.object("eventCalendar/" + user.address.estate + "/" + user.$key + "/" + event.id);
...
/***/ "../../../../../src/app/components/basic/event-calendar/shared/convert-event.service.ts":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return ConvertEventService; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__angular_core__ = __webpack_require__("../../../core/@angular/core.es5.js");
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
...
var ConvertEventService = (function () {
    function ConvertEventService() {
    }
    ConvertEventService.prototype.convertToCalendarEvent = function (event) {
        return {
            title: event.title,
    ...
    return ConvertEventService;
}());
ConvertEventService = __decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["Injectable"])(),
    __metadata("design:paramtypes", [])
], ConvertEventService);

//# sourceMappingURL=convert-event.service.js.map

/***/ }),

/***/ "../../../../../src/app/components/basic/event-calendar/shared/event.ts":
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var userEvent = (function () {
    ...
    function userEvent(title, description) {
        // reminder: boolean;
        this.draggable = true;
        this.title = title;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__angular_core__ = __webpack_require__("../../../core/@angular/core.es5.js");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_angularfire2_database__ = __webpack_require__("../../../../angularfire2/database/index.js");
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__convert_event_service__ = __webpack_require__("../../../../../src/app/components/basic/event-calendar/shared/convert-event.service.ts");
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var ListEventService = (function () {
    function ListEventService(db, convertSvc) {
        this.db = db;
        this.convertSvc = convertSvc;
    }
    ListEventService.prototype.fetchEvent = function (user, exportData) {
        var _this = this;
        var calendarEventList = [];
        var dbRef = this.db.list("eventCalendar/" + user.address.estate + "/" + user.$key + "/");
        dbRef.subscribe(function (eventDbList) {
            calendarEventList = [];
            eventDbList.forEach(function (event) {
                ...
                //        calendarEventList.push(convertedEvent)
                //      }
                var convertedEvent = _this.convertSvc.convertToCalendarEvent(event);
                calendarEventList.push(convertedEvent);
            });
...
ListEventService = __decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["Injectable"])(),
    __metadata("design:paramtypes", [typeof (_a = typeof __WEBPACK_IMPORTED_MODULE_1_angularfire2_database__["a" /* AngularFireDatabase */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_1_angularfire2_database__["a" /* AngularFireDatabase */]) === "function" && _a || Object, typeof (_b = typeof __WEBPACK_IMPORTED_MODULE_2__convert_event_service__["a" /* ConvertEventService */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_2__convert_event_service__["a" /* ConvertEventService */]) === "function" && _b || Object])
], ListEventService);
